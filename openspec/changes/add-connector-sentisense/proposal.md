...
need the same handful of reads over and over: what the price is, how the
crowd and the news feel about a ticker, what insiders, members of Congress
and 13F institutions have been buying, where Wall Street's targets sit,
what options positioning says, and when the company reports next.
...
fit mechanically: one base URL, one header key, fourteen synchronous GETs,
and three flat per-call credit classes. No new engine capability is necessary.

## What Changes

- **connectors/sentisense**: 14 endpoints against
  `https://app.sentisense.ai/api`, auth `presets.auth.header("X-SentiSense-API-Key")`:
  - per ticker: `/v1/stocks/{ticker}/quote`, `/v1/stocks/{ticker}/sentiment`,
    `/v1/rating/{ticker}`, `/v1/analyst/{ticker}/consensus`,
  ...
  - market-wide: `/v2/market-mood`, `/v1/insider/cluster-buys`,
    `/v1/documents/stories/search`, `/v1/calendar/earnings`,
...
- **The def is SentiSense's rate card.** SentiSense meters in SentiSense
  ...
  provider's PER_CALL of 1 is the LOOKUP class (quote, entity search,
  earnings calendar, market mood); the ANALYTICS endpoints (sentiment,
  rating, stories by ticker, story search, analyst consensus) override it
  with 2; the ALTERNATIVE-DATA endpoints (insider trades, cluster buys,
  congressional trades, 13F holders, options summary) with 4. The API
  reports no per-response meter, so there is no `consolidate`: the derived
  fold settles each run at its class. The $/credit conversion is the
  broker card's job.
- **Faithful mirrors, strict.** Each query-param mirror carries the
  ...
  characters, the `type`, `time`, `week`, `sortBy` and `sortDir` enums,
  `YYYY-MM-DD` dates. Caps the API clamps rather than rejects (`limit`,
...
- **Errors.** A provider `output.fromError` digests `{error, message,
  ...
  symbol) and `seeInstead` (the endpoint for an ETF) are lifted because they
  are the caller's next move.
...
- Real recordings for all 14 endpoints (2026-09-24), a recorded 404
...
- The rest of the SentiSense surface (historical metrics, fundamentals
  history, the screener, ETF endpoints, story detail) is not ported in this
  change. The fourteen here cover the common research loop; the screener's
  plan body in particular deserves its own change.
- No dollar conversion in the doc: the pool is SentiSense credits.

## Impact

New connector tree, 14 new ids in `connectors/ids.lock.json`, and three
